    ...
    <div>
      ...
      <div className="table-responsive">
        {recetteList && recetteList.length > 0 ? (
            ...
            <tbody>
              {recetteList.map((recette, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{recette.date ? <TextFormat type="date" value={recette.date} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{recette.user ? recette.user.login : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() => (window.location.href = `/recette/${recette.id}/delete`)}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Recettes found</div>
        )}
      </div>
    </div>
  );
